...
app.get("/api/employees", (req, res) => {
  db.all("SELECT * FROM employees", [], (_, rows) => res.json(rows));
});
...
app.post("/api/employees", (req, res) => {
  ...
  db.run(
    ...
    err => {
      if (err) return res.status(409).json({ error: "Employee already exists" });
...
app.delete("/api/employees/:id", (req, res) => {
  db.run("DELETE FROM employees WHERE employeeId = ?", [req.params.id], function () {
...
app.get("/api/dashboard", (req, res) => {
  db.get("SELECT COUNT(*) as totalEmployees FROM employees", [], (_, emp) => {
    db.get(
...
app.listen(PORT, () => console.log(`HRMS Lite running on port ${PORT}`));
